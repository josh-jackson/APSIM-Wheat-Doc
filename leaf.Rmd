...
```{block, type="rmdimportant", echo=TRUE}
In the previous version of APSIM-Wheat model,  the x-value of leaf area profile is `RelativeNodeAppearance`. However `RelativeNodeAppearance` is suddently increased at stage `Terminal Spikelet` which is caused by resetting of final node number. 
```
