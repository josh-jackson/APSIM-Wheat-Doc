...
Leaf area index (LAI) and coverage are calculated for green leaf ($\text{LAI}_{g}$ and $C_g$), dead leaf ($\text{LAI}_{d}$ and $C_d$), and total leaf ($\text{LAI}_{t}$ and $C_t$).
...
The coverages are calculated from LAI and extinction coefficient for green leaf ($k_{g}$) and dead leaf ($k_{d}$). 
...
<div class="fig-input">
```{r k-water-stress, fig.asp = 0.75, fig.cap='Extinction coefficient as a function of water stress'}
