...
Population of main stem number ($P_{ms}$) is calculated according to plant population ($P$) and primary bud number ($N_{bud}$) with default value 1. The unit of $P_{ms}$ is per square meter.
$$
    P_{ms}=P \times N_{bud}
$$
...
<div class="fig-output">
```{r plant-popultion, fig.cap='The total population of main stem and all stems'}
...
As the primary bud number is equal to 1, the plant total node number and total primary node number are the same (Figure \@ref(fif:plant-node-number)). 
