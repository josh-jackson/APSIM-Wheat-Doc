...
plot_report(
    df %>% filter(Wheat.Phenology.Stage < 10),
            x_var, y_cols, x_lab = 'Stage', y_lab = 'Total population of main stem and all stems')
...
<div class="fig-input">
```{r mortality-growth-rate, fig.cap='Tiller mortality as a function of moving mean tiller growth rate'}
...
x_var <- 'Wheat.Phenology.Stage'
plot_report(df, x_var, y_cols, x_lab = 'Stage', y_lab = 'Moving tiller growth rate')
...
y_cols <- c(
    ...
    'Wheat.Structure.TotalStemPopn')
x_var <- 'Wheat.Phenology.Stage'
plot_report(df, x_var, y_cols, x_lab = 'Stage', y_lab = 'Three factors to calculate the tiller growth rate.',
            panel = TRUE)
...
As the primary bud number is equal to 1, the plant total node number and total primary node number are the same (Figure \@ref(fig:plant-node-number)). Figure \@ref(fig:plant-nod-number) show the total node number as the results of tiller appearing and mortality.
...
plot_report(df,
            x_var, y_cols, x_lab = 'Stage', y_lab = 'Total plant node number')
